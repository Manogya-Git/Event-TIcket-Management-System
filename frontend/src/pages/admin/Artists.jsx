...
const Artists = () => {
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(`${BASE_URL}/artists/${selectedArtist.slug}/`, {
        headers: { Authorization: `Bearer ${accessToken}` },
      });
      setRows((currentRows) =>
        currentRows.filter((row) => row.slug !== selectedArtist.slug),
      );
      setSelectedArtist(null);
    } catch (requestError) {
      console.error("Failed to delete artist:", requestError);
      setError("Unable to delete artist.");
    }
  };

  if (loading) {
    return <p className="p-8 text-slate-600">Loading artists...</p>;
  }
    ...
    <div className="min-h-screen bg-slate-50 p-8">
      <div className="mb-6 flex items-center justify-between gap-4">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-bold text-slate-900">Artists</h1>
        </div>
        <Button
          variant="contained"
          startIcon={<AddIcon />}
      ...
      <Dialog
        ...
        aria-labelledby="delete-artist-dialog-title"
      >
        <DialogTitle id="delete-artist-dialog-title">Delete Artist</DialogTitle>
        <DialogContent>
          <DialogContentText>
            Are you sure you want to delete {selectedArtist?.name}? This action
            cannot be undone.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
                    ...
                    >
                      ...
                      <TableCell sx={{ borderBottom: "1px solid #e2e8f0" }}>
                        <IconButton
                          ...
                          onClick={() =>
                            navigate(`/admin/artists/${row.slug}/edit`)
                          }
                          sx={{
                            color: "#0f172a",
